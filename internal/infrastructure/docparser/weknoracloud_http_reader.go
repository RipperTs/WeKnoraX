...
func NewWeKnoraCloudSignedDocumentReader(appID, apiKey string) (*WeKnoraCloudSignedDocumentReader, error) {
	if appID == "" {
		return nil, fmt.Errorf("Cloud service appID is required")
	}
	if apiKey == "" {
		return nil, fmt.Errorf("Cloud service apiKey is required")
	}
...
func (p *WeKnoraCloudSignedDocumentReader) ListEngines(ctx context.Context, overrides map[string]string) ([]types.ParserEngineInfo, error) {
	return []types.ParserEngineInfo{{
		...
		Description: "Cloud service signed docreader",
...
func (p *WeKnoraCloudSignedDocumentReader) Read(ctx context.Context, req *types.ReadRequest) (*types.ReadResult, error) {
	logger.Infof(ctx, "[Cloud service] read start file=%q type=%q engine=%q hasURL=%v contentLen=%d requestID=%q",
	...
	if err != nil {
		logger.Errorf(context.Background(), "[Cloud service] marshal read request: %v", err)
	...
	if err != nil {
		logger.Errorf(context.Background(), "[Cloud service] signed read request: %v", err)
		return nil, err
	}
	resp, err := p.client.Do(httpReq)
	if err != nil {
		logger.Errorf(context.Background(), "[Cloud service] http read request failed: %v", err)
		return nil, fmt.Errorf("http read failed: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusAccepted && resp.StatusCode != http.StatusOK {
		bodyBytes, _ := io.ReadAll(resp.Body)
		logger.Errorf(context.Background(), "[Cloud service] http read unexpected status %d: %s", resp.StatusCode, string(bodyBytes))
	...
	if err := json.NewDecoder(resp.Body).Decode(&submit); err != nil {
		logger.Errorf(context.Background(), "[Cloud service] decode read submit response: %v", err)
		return nil, fmt.Errorf("http decode read submit response: %w", err)
	}
	if strings.TrimSpace(submit.TaskID) == "" {
		logger.Errorf(context.Background(), "[Cloud service] submit response missing task_id (status=%q message=%q)", submit.Status, submit.Message)
		return nil, fmt.Errorf("cloud document reader submit response missing task_id")
	}
	logger.Infof(ctx, "[Cloud service] task submitted task_id=%s file=%q type=%q", submit.TaskID, req.FileName, req.FileType)
...
func (p *WeKnoraCloudSignedDocumentReader) pollTaskResult(ctx context.Context, taskID string) (*types.ReadResult, error) {
	...
	for {
		...
		if err != nil {
			logger.Errorf(context.Background(), "[Cloud service] poll signed request task_id=%s: %v", taskID, err)
			return nil, err
		}
		resp, err := p.client.Do(httpReq)
		if err != nil {
			logger.Errorf(context.Background(), "[Cloud service] http poll task_id=%s failed: %v", taskID, err)
		...
		func() {
			...
			if resp.StatusCode != http.StatusOK {
				...
				logger.Errorf(context.Background(), "[Cloud service] poll task_id=%s status %d: %s", taskID, resp.StatusCode, string(bodyBytes))
				return
			}
			if decodeErr := json.NewDecoder(resp.Body).Decode(&taskResp); decodeErr != nil {
				err = fmt.Errorf("http decode task response: %w", decodeErr)
				logger.Errorf(context.Background(), "[Cloud service] poll task_id=%s decode response: %v", taskID, decodeErr)
			}
		}()
		if err != nil {
			return nil, err
		...
		case "completed":
			if taskResp.Result == nil {
				logger.Infof(ctx, "[Cloud service] task_id=%s completed with no result payload", taskID)
				return &types.ReadResult{}, nil
			}
			res := fromHTTPReadResponse(taskResp.Result)
			if res.Error != "" {
				logger.Errorf(ctx, "[Cloud service] task_id=%s completed with result.error: %s", taskID, res.Error)
			} else {
				logger.Debugf(ctx, "[Cloud service] task_id=%s completed ok markdownLen=%d", taskID, len(res.MarkdownContent))
			}
			return res, nil
		case "failed":
			if taskResp.Error != "" {
				logger.Errorf(context.Background(), "[Cloud service] task_id=%s failed: %s", taskID, taskResp.Error)
				return nil, fmt.Errorf("cloud document reader task failed: %s", taskResp.Error)
			}
			logger.Errorf(context.Background(), "[Cloud service] task_id=%s failed: %s", taskID, taskResp.Message)
			return nil, fmt.Errorf("cloud document reader task failed: %s", taskResp.Message)
		case "cancelled":
			if taskResp.Error != "" {
				logger.Errorf(context.Background(), "[Cloud service] task_id=%s cancelled: %s", taskID, taskResp.Error)
				return nil, fmt.Errorf("cloud document reader task cancelled: %s", taskResp.Error)
			}
			logger.Errorf(context.Background(), "[Cloud service] task_id=%s cancelled", taskID)
			return nil, fmt.Errorf("cloud document reader task cancelled")
		}
		if err := pollCtx.Err(); err != nil {
			logger.Errorf(ctx, "[Cloud service] poll task_id=%s aborted before sleep: %v", taskID, err)
			return nil, err
		}
		...
		select {
		case <-pollCtx.Done():
			logger.Errorf(ctx, "[Cloud service] poll task_id=%s stopped: %v", taskID, pollCtx.Err())
...
func (p *WeKnoraCloudSignedDocumentReader) newSignedRequest(ctx context.Context, method, url string, body []byte) (*http.Request, error) {
	...
	if err != nil {
		logger.Errorf(context.Background(), "[Cloud service] http new request %s %s: %v", method, url, err)
