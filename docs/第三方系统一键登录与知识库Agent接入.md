...
- 首次连接时由用户确认权限，后续无感打开；
- 使用独立的用户连接凭证，通过 REST、MCP 或 Skill 检索已授权空间中的知识库；
...
| 权限计算 | 用户授权空间 ∩ 空间策略 ∩ 应用当前配置 ∩ 用户当前成员与知识库访问权 |
...
未创建空间策略时，默认允许应用配置中的 Scope。知识库不再单独配置白名单：用户授权空间后，该空间中用户当前可访问的知识库会动态进入凭证范围。
...
// 将 state、codeVerifier、当前第三方用户 ID 和创建时间保存在服务端会话中。
```
...
如果用户未登录，WeKnora 会记住完整授权地址并进入登录流程；部署启用 OIDC 时会自动发起现有 OIDC 登录。登录完成后返回授权页。首次连接展示确认页，用户可以多选自己当前加入且允许该应用接入的空间，至少选择一个空间后完成授权。

第三方不传递也不保存 WeKnora 空间 ID。空间归属完全由当前登录用户在 WeKnora 授权页中选择，避免把用户级空间错误配置为应用级固定参数。

同一应用和用户已有有效连接，且请求 Scope 没有扩大、至少一个原授权空间仍有效时，授权页会自动复用原连接并直接回调，不再重复确认。

用户加入新空间后，该空间不会自动进入已有连接。第三方需要让用户调整授权空间时，在授权地址增加 `prompt=consent`，WeKnora 会重新展示空间多选页；确认后以本次选择完整替换原授权空间。
...
```json
{
  ...
  "launch_path": "/integrations/launch/0d2c1ff4-1b55-4d98-9830-f056ccb9e643",
...
```text
(third_party_user_id, weknora_application/client_id)
```

至少保存 `access_token`、`connection_id`、`launch_path`、`scopes` 和更新时间。`access_token` 应进入密钥管理系统或加密列，不写日志、不写分析事件、不进入浏览器 Cookie/localStorage。
...
必须使用 Token 响应中原样返回的 `launch_path`。WeKnora 会校验当前登录用户就是连接所有者，并实时显示所有有效授权空间及其中可访问的知识库：
...
省略 `knowledge_base_ids` 时，仅对 `wkic_` 集成凭证自动使用所有已授权空间中的完整有效知识库范围。普通 JWT 或普通 API Key 仍需明确提供搜索目标。
...
应用、用户连接及当前有效授权空间的策略都包含 `knowledge.chat` 时，凭证才可以创建该连接隔离的会话并调用 `/knowledge-chat`。该权限不允许读取 Agent 配置、调用 `/agent-chat` 或执行自定义 Agent 工具，也不授予模型、知识库和内容管理权限。

调用 `/knowledge-chat` 时可以省略 `knowledge_base_ids`，服务端会使用所有已授权空间中的完整有效知识库范围；如果该范围已为空，请求会直接失败，不会退化为脱离知识库的纯模型问答。
...
2. 第三方应用仍启用；
3. 至少一个用户授权的空间策略仍启用；
4. Scope 仍存在于连接、应用和有效授权空间策略的交集；
5. 用户账号仍启用且仍是对应授权空间的成员；
6. 知识库仍位于有效授权空间，并且用户当前仍有访问权；
7. 授权空间接收的跨空间共享知识库仍具有 Viewer 或更高权限。
...
| `GET` | `/integrations/launch/{connection_id}` | 浏览器 WeKnora 会话 | 后续无感打开 |
...
- 已执行 PostgreSQL `000090` 或 SQLite `000016` 迁移；旧连接会失效并要求用户重新授权；
- WeKnora 和回调地址按部署网络配置 HTTP 或 HTTPS；
- 回调地址与注册值完全一致；
- `client_secret` 和 `wkic_` 存入服务端密钥系统；
- `state` 一次性、校验归属并设置短超时；
- PKCE 固定使用 S256；
- 第三方数据库按用户和 `client_id` 隔离连接记录，不保存固定空间 ID；
