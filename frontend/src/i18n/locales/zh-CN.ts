export default {
  ...
  userProfile: {
    ...
    changePassword: {
      label: '修改密码',
      description: '设置新的登录密码。修改成功后所有设备上的登录状态将失效，需要重新登录。',
      newLabel: '新密码',
  ...
  integrations: {
    ...
    claw: {
      title: '知识库技能',
      subtitle: '通过知识库 REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装知识库 Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合知识库自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用知识库桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和云服务凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在知识库与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与知识库保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  datasource: {
    ...
    noResourcesDesc_confluence: '请确认该账号有可读取的 Confluence 空间，或目标站点已开放匿名读取',
    ...
    connectorDesc: {
      ...
      confluence: '同步 Confluence 空间中的页面和文档附件',
      yuque: '同步语雀知识库中的文档',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      model: '选择智能体使用的大语言模型',
      titleModel: '用于自动生成会话标题的模型，留空则复用主对话模型',
  ...
  model: {
    ...
    editor: {
      ...
      sortOrderLabel: '排序值',
      sortOrderPlaceholder: '默认 100',
      sortOrderDesc: '用于用户端模型选择列表排序，数字越小越靠前；数字相同时按创建时间倒序。',
      ...
      providers: {
        ...
        weknoracloud: {
          label: '云模型服务',
          description: '云模型服务，支持 chat、embedding、rerank、vlm 模型'
        },
        requesty: {
          label: 'Requesty',
    ...
    globalSettings: {
      ...
      runtime: {
        ...
        purge: {
          ...
          liveWarning: '此操作可能影响多个空间，并终止关联业务及同一文档的其他队列任务；Wiki 会联动处理同一知识库的关联任务。已写入或已删除的数据不会回滚。知识库删除和索引删除任务会保留，以完成资源清理。其他运行中的任务需等待执行程序退出，未停止或处理失败的任务会明确提示。新提交的任务仍可进入队列。',
          historyWarning: '普通历史任务仅清除队列记录；若包含此前清理未完成的隔离任务，会先重试必要收尾。原任务不会重新执行，文档内容不会回滚。',
          confirmButton: '确认执行',
  ...
  knowledgeEditor: {
    ...
    apiAccess: {
      title: 'API 接入',
      description: '通过 API 将外部系统的文档同步到当前知识库，支持新增、更新、查询和删除。',
      createTitle: '创建接入密钥',
      ...
      copyExample: '复制调用示例',
      exampleCopySuccess: '调用示例已复制',
      sourceIdHint: '来源系统的稳定标识，最长 128 个字符，例如 policy-system。',
      externalIdHint: '文档在来源系统中的唯一标识，最长 512 个字符。三种操作应使用相同标识。',
      ...
      authStep: '携带认证请求头',
      callStep: '选择接口并调用',
      chooseOperation: '选择接口操作',
      upsertTitle: '新增／更新',
      upsertDescription: '上传文档文件。相同来源和文档标识会更新已有文档，首次上传则创建新文档。',
      upsertResponseHint: '202 表示已受理，解析仍在后台进行，请通过 GET 查询状态。action 为 created 或 updated；内容未变化且正在处理或已完成时返回 200 和 skipped。',
      getTitle: '查询状态',
      getDescription: '根据来源系统和文档标识查询解析进度、当前文档 ID 和处理结果。',
      getResponseHint: 'parse_status 为 completed 表示处理完成；failed 时查看 error_message。文档不存在返回 404。查询响应不包含 action 字段。',
      deleteTitle: '删除文档',
      deleteDescription: '来源文档被删除或下架时，使用相同的来源系统和文档标识同步删除知识库中的文档。',
      deleteResponseHint: '删除成功返回 200 和 deleted；文档不存在仍返回 200 和 skipped。此操作删除对应文档及关联索引，响应中的 parse_status 是删除前的解析状态。',
      ...
      required: '是',
      optional: '否',
      fileHint: '上传的文档文件。格式与大小限制以服务端配置为准；更新时也需提交完整文件。',
      metadataHint: 'JSON 对象，值必须为字符串，例如部门、分类。元数据变化也会触发更新。',
      requestExampleTitle: '调用示例',
      responseExampleTitle: '响应示例',
      exampleHint: '地址已带入当前站点和知识库。请替换 API_KEY、来源标识和文档标识；上传时还需替换本地文件路径。',
      statusesTitle: '处理状态说明',
      statusPending: '等待处理',
      statusProcessing: '正在解析、分块或向量化',
      statusFinalizing: '正在生成摘要、问题或知识图谱',
      statusCompleted: '全部处理完成',
      statusFailed: '处理失败，查看 error_message；可重新 PUT 提交文档',
      statusCancelled: '处理已取消，可重新 PUT 提交文档',
      errorsTitle: '常见错误',
      error400: '参数、文件格式、大小或元数据不符合要求',
      error401: '未提供密钥或密钥无效',
      error403: '密钥无当前知识库的 ingest 权限，或访问范围不允许',
      error404: '知识库不存在，或 GET 查询的文档不存在',
      error409: '文档与现有记录冲突',
      error429: '当前空间的存储配额已用尽',
      error500: '服务端处理失败，请检查服务日志',
      syncRulesTitle: '重复调用与同步规则',
      identityRule: '同一知识库内，以 source_id + external_id 唯一定位文档，新增、更新、查询和删除均使用这组标识。',
      skipRule: '文件名、文件内容和元数据均未变化，且文档正在处理或已完成时，重复 PUT 返回 skipped；失败或取消后重新提交会再次处理。',
      updateRule: '更新会创建新文档并清理旧文档，knowledge_id 会变化。外部系统应持续使用 source_id 和 external_id 进行同步。',
      orderRule: '同一文档的请求串行执行，但服务端无法识别业务版本的新旧。来源系统应按文档变更顺序发送请求。',
      columns: {
        name: '名称',
        key: '密钥',
        createdAt: '创建时间',
  ...
  thirdPartyIntegration: {
    system: {
      navLabel: '第三方应用',
      title: '第三方应用',
      description: '注册可信第三方系统，并管理其回调地址和可申请权限。',
      create: '注册应用',
      edit: '编辑应用',
      formDescription: 'Client Secret 仅在创建或轮换后显示一次。',
      noDescription: '暂无描述',
      redirectCount: '{count} 个回调地址',
      testConnection: '测试连接',
      testSuccess: '{count} 个回调地址均可访问，HTTP 状态码：{statuses}',
      testFailed: '连接测试失败，已连通 {reachable}/{total}。{url}：{error}',
      testRequestFailed: '连接测试请求失败',
      testUnknownError: '未知错误',
      deleteTitle: '删除第三方应用',
      deleteConfirm: '确定删除“{name}”？相关空间策略、用户授权、连接和访问凭证将同时删除，且无法恢复。',
      deleted: '第三方应用已删除',
      deleteFailed: '删除第三方应用失败',
      rotateSecret: '轮换密钥',
      rotateTitle: '轮换 Client Secret',
      rotateConfirm: '确定轮换“{name}”的 Client Secret？旧 Secret 将立即失效。',
      rotateFailed: '密钥轮换失败',
      emptyTitle: '尚未注册第三方应用',
      emptyDescription: '注册后，第三方系统即可发起用户授权连接。',
      required: '请填写应用名称和至少一个回调地址',
      loadFailed: '加载第三方应用失败',
      saved: '应用已保存',
      saveFailed: '保存应用失败',
    },
    fields: {
      ...
      description: '应用说明',
      redirectUris: '回调地址',
      redirectUrisPlaceholder: '每行一个 HTTP 或 HTTPS 地址，例如：http://agent.internal/callback',
      redirectUrisHint: '允许使用 HTTP 或 HTTPS，地址必须是完整 URL。',
      scopes: '可申请权限',
      enabled: '启用应用',
      enabledHint: '关闭后将停止新授权，已有凭证也会立即失效。',
    },
    scopes: {
      read: '读取与检索知识库',
      chat: '基于知识库问答',
    ...
    secret: {
      title: '保存应用凭证',
      copy: '复制 Client Secret',
      warning: 'Client Secret 仅显示这一次，请立即保存到第三方服务端的安全配置中。',
    },
    tenant: {
      navLabel: '第三方连接',
      title: '第三方连接',
      description: '管理当前空间允许接入的应用，以及你授权给第三方的多空间连接。',
      applications: '空间应用策略',
      applicationsDescription: '空间管理员可控制应用是否能够访问当前空间及其可用权限。',
      configure: '配置',
      unrestrictedSummary: '{scopes} · 用户授权空间后动态访问知识库',
      empty: '系统管理员尚未注册可用应用',
      loadFailed: '加载第三方连接失败',
      saved: '空间策略已保存',
      saveFailed: '保存空间策略失败',
    },
    connections: {
      title: '我的已授权连接',
      description: '连接凭证由第三方服务端保管；撤销后会立即失效。',
      connected: '已连接',
      unavailable: '已停用',
      summary: '{spaces} 个空间 · {count} 个知识库 · {scopes}',
      unavailableSummary: '应用已停用，或已授权空间当前均不可用。',
      open: '打开',
      revoke: '撤销',
      revokeConfirm: '确定撤销与“{name}”的连接？第三方 Agent 将立即无法访问。',
      empty: '你尚未授权任何第三方应用',
      revoked: '连接已撤销',
      revokeFailed: '撤销连接失败',
    },
    policy: {
      title: '配置 {name}',
      description: '空间策略只会收紧系统应用配置，不会扩大权限。',
      enabled: '允许在当前空间使用',
      enabledHint: '关闭后，已有连接将立即停止访问当前空间。',
    },
    authorization: {
      loading: '正在校验连接请求…',
      errorTitle: '无法完成连接',
      eyebrow: '第三方应用授权',
      title: '允许“{name}”连接你的知识库？',
      credentialNotice: '授权后会向该应用签发独立凭证，不会共享你的登录 Token 或 OIDC 凭证。',
      permissions: '应用将获得',
      readTitle: '读取与检索已选空间的知识库',
      readDescription: '可读取所选空间中你当前有权访问的知识库，权限变化会实时生效。',
      chatTitle: '基于知识库发起问答',
      chatDescription: '允许应用调用知识问答接口，但不能写入、修改或删除内容。',
      selectWorkspaces: '选择授权空间',
      selectWorkspacesHint: '可同时选择多个空间，以后可重新授权或撤销整个连接。',
      noWorkspaces: '当前没有允许此应用接入的空间',
      footerHint: '系统会在每次请求时重新计算有效权限。',
      deny: '拒绝',
      allow: '确认连接',
    ...
    launch: {
      ...
      successLabel: '授权已完成',
      title: '连接成功',
      description: '“{name}”已获得你确认的访问权限，可以从下方进入知识库。',
      workspaceCount: '已授权空间',
      knowledgeBaseCount: '可访问知识库',
      workspaces: '已授权空间',
      knowledgeBases: '选择要打开的知识库',
      openHint: '点击进入知识库',
      empty: '已授权空间当前没有可访问的知识库，请重新授权或联系空间管理员。',
      loadFailed: '加载连接失败',
    },
  },
  common: {
  ...
  auth: {
    ...
    redirectingToSSO: '正在跳转到认证中心...',
    orContinueWith: '或使用以下方式继续',
    oidcLoginFailed: 'OIDC 登录失败',
    quickLogin: '快捷登录',
    quickLoginDescription: '选择所属企业，使用统一身份认证登录',
    passwordLogin: '账号密码登录',
    fushunSSOProvider: '抚顺新钢铁',
    fushunSSOLogin: '抚顺新钢铁 SSO 登录',
    fushunSSODescription: '使用抚顺新钢铁统一身份认证登录',
    jianlongSSOProvider: '建龙集团',
    jianlongSSOLogin: '建龙集团 SSO 登录',
    jianlongSSODescription: '使用建龙集团统一身份认证登录',
    username: '用户名',
    loginAccount: '账号',
    email: '邮箱',
    password: '密码',
    ...
    registerSubtitle: '创建账户并开始使用知识库',
    loginAccountPlaceholder: '输入用户名或邮箱地址',
    emailPlaceholder: '输入邮箱地址',
    passwordPlaceholder: '输入密码（8-32个字符，包含字母和数字）',
    confirmPasswordPlaceholder: '再次输入密码',
    usernamePlaceholder: '输入用户名',
    loginAccountRequired: '请输入用户名或邮箱地址',
  ...
  settings: {
    ...
    sandbox: {
      ...
      weknoraTemplateTitle: '标准沙箱模板',
      weknoraDockerImage: 'Sandbox 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      localRuntimeWarning: '本地进程会直接在服务器上执行技能脚本，不具备容器或 MicroVM 隔离。仅建议在可信的开发空间使用。',
      weknoraTemplateOverview: '系统负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      dockerTlsCertPathHelp: '服务器所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被系统回收，会话继续时重建。留空按 1800 秒。',
    ...
    storage: {
      ...
      configurable: '可配置',
      pathPrefix: '路径前缀（可选）',
      pathPrefixPlaceholder: '如 knowledge/images',
      prefixPlaceholder: '如 knowledge',
    ...
    weknoraCloud: {
      title: '云服务',
      description: '配置云服务的 APPID 和 APPSECRET 凭证。凭证用于模型服务和文档解析引擎。',
      viewDocs: '查看文档',
      ...
      modelsSection: {
        title: '云模型接入',
        descReady: '将云服务提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  agent: {
    ...
    editor: {
      ...
      titleModel: '标题生成模型',
      titleModelPlaceholder: '留空则复用主对话模型',
      systemPrompt: '系统提示词',
