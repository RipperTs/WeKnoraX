export default {
  ...
  knowledgeEditor: {
    ...
    apiAccess: {
      title: 'Доступ к API',
      description: 'Создайте отдельный ключ, позволяющий внешней системе записывать документы только в эту базу знаний.',
      endpointLabel: 'Эндпоинт синхронизации документов',
      scopeNotice: 'Ключ привязан к этой базе знаний и предоставляет только разрешение ingest.',
      ...
      quickStartDescription: 'Скопируйте ключ и загрузите первый документ с помощью примера ниже.',
      copyExample: 'Копировать пример',
      exampleCopySuccess: 'Пример запроса скопирован',
      sourceIdHint: 'Постоянный идентификатор исходной системы.',
      externalIdHint: 'Уникальный идентификатор документа в этой системе. Повторная загрузка обновит документ.',
