export default {
  ...
  userProfile: {
    ...
    description: 'Просмотр данных аккаунта (ID, имя пользователя, имя, email, дата регистрации) и изменение имени или пароля.',
    name: {
      label: 'Имя',
      description: 'Используется для отображения аккаунта; поле можно оставить пустым',
      empty: 'Не указано',
      edit: 'Изменить имя',
      ...
      failed: 'Не удалось обновить имя',
    },
    changePassword: {
      label: 'Сменить пароль',
      description: 'Задайте новый пароль для входа. После успешной смены все сессии будут отозваны, потребуется повторный вход.',
  ...
  integrations: {
    ...
    claw: {
      title: 'Навык работы с базой знаний',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API базы знаний — загрузки, URL, Markdown и поиск.',
      ...
      installCtaHint: 'Установка навыка работы с базой знаний · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для вашей базы знаний: вопросы в боковой панели, сохранение веб-страниц и Markdown-заметок прямо во время просмотра.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В настольном приложении базы знаний задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными облачного сервиса.',
      ...
      hmacSecretDesc: 'Тот же secret, что в системе базы знаний; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
  ...
  datasource: {
    ...
    noResourcesDesc_confluence: 'Убедитесь, что учётная запись может читать хотя бы одно пространство Confluence или включён анонимный доступ',
    ...
    field: {
      ...
      confluencePassword: 'Пароль (необязательно)',
      baseUrl: 'Base URL',
    ...
    connectorDesc: {
      ...
      confluence: 'Синхронизация страниц и вложений документов из пространств Confluence',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      titleModel: 'Модель для автоматического создания заголовков сессий. Пустое значение — использовать основную модель чата.',
  ...
  kbSettings: {
    parser: {
      ...
      engines: {
        ...
        weknoracloud: {
          name: 'Облачный парсинг',
          desc: 'Парсинг документов через облачный сервис'
        },
        paddleocr_vl_cloud: {
  ...
  modelSettings: {
    title: 'Управление моделями платформы',
    description: 'Системные администраторы управляют моделями ИИ, доступными всем пространствам',
    copySuffix: ' копия',
    builtinTag: 'Платформа',
    ...
    builtinModels: {
      ...
      descriptionAdmin: 'Созданные здесь модели видны всем пространствам; конфигурацией и учетными данными управляют только системные администраторы.',
  ...
  model: {
    ...
    defaultTag: 'По умолчанию',
    addModelInSettings: 'Перейти в системное администрирование для добавления моделей',
    ...
    editor: {
      ...
      sortOrderPlaceholder: 'По умолчанию: 100',
      sortOrderDesc: 'Определяет порядок моделей в списках выбора. Меньшие значения идут первыми; при равенстве новые модели идут первыми.',
      ...
      providers: {
        ...
        weknoracloud: {
          label: 'Облачные модели',
          description: 'Облачный сервис базы знаний для моделей chat, embedding, rerank и VLM'
        },
        requesty: {
          label: 'Requesty',
  ...
  systemTenants: {
    ...
    description: 'Просматривайте пространства всех пользователей и использование хранилища, увеличивайте квоты.',
    total: 'пространств',
    search: 'Поиск',
    searchPlaceholder: 'Название, ID, имя или email владельца',
    noOwner: 'Нет владельца',
    unlimited: 'Без ограничений',
    empty: 'Подходящие пространства не найдены',
    columns: {
    ...
    increase: {
      action: 'Увеличить квоту',
      current: 'Текущая квота',
      amount: 'Добавить объём (ГБ)',
      after: 'Квота после увеличения',
      hint: 'Квоту можно только увеличивать. Введите положительное целое число; 1 ГБ = 1024 МБ. Объём добавляется к актуальной квоте при отправке.',
      success: 'Квота увеличена. Новый общий объём: {quota}'
    },
    auditTarget: 'Квота пространства: {name} (ID: {id})',
    auditDiff: 'Добавлено {increase} ГБ: {before} ГБ → {after} ГБ'
  },
  systemUsers: {
    navLabel: 'Управление пользователями',
    title: 'Управление пользователями',
    description: 'Просмотр учетных записей и управление входом, паролями и правами системного администратора.',
    total: 'пользователей',
    searchPlaceholder: 'Поиск по имени или эл. почте',
    refresh: 'Обновить список',
    retry: 'Повторить',
    empty: 'Пользователей пока нет',
    emptyFiltered: 'Нет пользователей, соответствующих фильтрам',
    loadFailed: 'Не удалось загрузить пользователей',
    columns: { user: 'Пользователь', status: 'Статус', role: 'Права платформы', createdAt: 'Регистрация', actions: 'Действия' },
    status: { all: 'Все статусы', active: 'Активен', disabled: 'Отключен' },
    role: { systemAdmin: 'Системный администратор', regular: 'Обычный пользователь' },
    actions: {
      more: 'Другие действия', resetPassword: 'Сбросить пароль', promoteAdmin: 'Назначить системным администратором',
      revokeAdmin: 'Отозвать права администратора', enable: 'Включить учетную запись',
      disable: 'Отключить учетную запись', revokeBeforeDisable: 'Сначала отзовите права администратора'
    },
    create: {
      action: 'Добавить пользователя', dialogTitle: 'Добавить пользователя', confirm: 'Создать пользователя',
      username: 'Имя пользователя', usernamePlaceholder: "2–50 символов, без {'@'}", email: 'Эл. почта',
      emailPlaceholder: 'Введите эл. почту пользователя', passwordMode: 'Начальный пароль',
      generated: 'Создать автоматически', manual: 'Задать вручную',
      generatedHint: 'Случайный пароль будет показан один раз после создания. Скопируйте и безопасно передайте его пользователю.',
      success: 'Учетная запись пользователя готова', failed: 'Не удалось создать пользователя',
      passwordResultTitle: 'Сохраните начальный пароль',
      passwordResultWarning: 'Пароль показывается только один раз и не может быть получен после закрытия.',
      copyPassword: 'Копировать пароль', copied: 'Пароль скопирован'
    },
    password: {
      dialogTitle: 'Сброс пароля пользователя',
      warning: 'Пароль для {email} будет сброшен, а все текущие сеансы завершены.',
      newPassword: 'Новый пароль', confirmPassword: 'Подтвердите новый пароль',
      placeholder: '8–32 символа, включая буквы и цифры', confirmPlaceholder: 'Введите новый пароль еще раз',
      confirm: 'Сбросить пароль', success: 'Пароль сброшен, текущие сеансы пользователя завершены',
      failed: 'Не удалось сбросить пароль'
    },
    validation: {
      usernameRequired: 'Введите имя пользователя', usernameLength: 'Имя пользователя должно содержать 2–50 символов',
      usernameAt: "Имя пользователя не может содержать {'@'}", emailRequired: 'Введите эл. почту',
      emailInvalid: 'Введите корректный адрес эл. почты', passwordRequired: 'Введите пароль',
      passwordLength: 'Пароль должен содержать 8–32 символа', passwordLetter: 'Пароль должен содержать букву',
      passwordNumber: 'Пароль должен содержать цифру', confirmRequired: 'Введите пароль еще раз',
      passwordMismatch: 'Пароли не совпадают'
    },
    confirm: {
      promote: { title: 'Предоставить права системного администратора', body: 'Назначить {email} системным администратором?', confirm: 'Предоставить права' },
      revoke: { title: 'Отозвать права системного администратора', body: 'Отозвать права системного администратора у {email}?', confirm: 'Отозвать права' },
      enable: { title: 'Включить учетную запись', body: 'Включить {email}? Пользователю потребуется войти снова.', confirm: 'Включить' },
      disable: { title: 'Отключить учетную запись', body: 'Отключить {email}? Пользователь немедленно выйдет и не сможет войти снова.', confirm: 'Отключить' }
    },
    messages: {
      promoteSuccess: 'Права системного администратора предоставлены', promoteFailed: 'Не удалось предоставить права системного администратора',
      revokeSuccess: 'Права системного администратора отозваны', revokeFailed: 'Не удалось отозвать права системного администратора',
      enableSuccess: 'Учетная запись включена', enableFailed: 'Не удалось включить учетную запись',
      disableSuccess: 'Учетная запись отключена', disableFailed: 'Не удалось отключить учетную запись'
    }
  },
  system: {
    ...
    globalSettings: {
      ...
      branding: {
        titlePlaceholder: 'Если пусто, отображается «企业知识库»',
        uploadLogo: 'Загрузить Logo',
        changeLogo: 'Заменить Logo',
        invalidLogoType: 'Поддерживаются только изображения PNG, JPG и WebP',
        logoTooLarge: 'Размер изображения Logo не должен превышать 1 МБ',
        logoReadFailed: 'Не удалось прочитать изображение Logo'
      },
      audit: {
        tabLabel: 'Журнал аудита',
        description: 'События платформы: создание и изменение статуса пользователей, выдача/отзыв прав системного администратора и изменения системных настроек. Сначала новые.',
        refresh: 'Обновить',
        retry: 'Повторить',
        loading: 'Загрузка…',
        end: 'Конец списка.',
        ...
        action: {
          ...
          'system.user_status_changed': 'Изменен статус пользователя',
          ...
          'system.queue_tasks_purged': 'Очистка задач очереди по состоянию'
        },
        columns: {
          time: 'Время',
          actor: 'Кто',
      ...
      bulkApply: {
        label: 'Увеличить квоты существующих пространств',
        tooltip: 'Увеличить конечные квоты ниже значения по умолчанию. Большие и неограниченные квоты сохраняются.',
        confirmBtn: 'Подтвердить применение',
        confirmBody: 'Увеличить конечные квоты ниже {value} ГБ до {value} ГБ. Квоты не будут уменьшены. Продолжить?',
        success: 'Квоты {count} пространств увеличены до {gb} ГБ',
      ...
      keyDescriptions: {
        branding: {
          site_title: 'Название сайта в верхней части левой панели, до 40 символов. Если поле пустое, используется встроенное название «企业知识库». Изменение применяется сразу.',
          site_logo_data_url: 'Logo сайта в верхней части левой панели. Поддерживаются PNG, JPG и WebP размером до 1 МБ; если поле пустое, используется встроенный значок папки.'
        },
        model: {
        ...
        tenant: {
          ...
          auto_accept_invitation: 'Если включено, зарегистрированный пользователь сразу добавляется в пространство без ручного принятия и ожидающего приглашения. Если отключено, пользователь должен подтвердить приглашение. Изменение применяется сразу.'
        },
        ssrf: {
        ...
        auth: {
          ...
          sso_auto_register_enabled: 'Управляет автоматическим созданием локального аккаунта при первом входе через SSO. Если включено, отсутствующий локальный пользователь создаётся автоматически; если отключено, через SSO могут входить только уже существующие локальные аккаунты. Изменение применяется сразу.'
        }
      },
      keyLabels: {
        branding: {
          site_title: 'Название сайта',
          site_logo_data_url: 'Logo сайта'
        },
        model: {
        ...
        tenant: {
          ...
          auto_accept_invitation: 'Автоматически принимать приглашения в пространство'
        },
        ssrf: {
        ...
        auth: {
          ...
          sso_auto_register_enabled: 'Авторегистрация новых пользователей SSO'
        }
      },
      runtime: {
        ...
        tasks: {
          ...
          guides: {
            ...
            completed: 'Показаны сохранённые недавние завершённые задачи. Очистить записи можно через меню действий строки очереди.'
          },
          states: {
            active: 'Активные',
            pending: 'Ожидают',
        ...
        purge: {
          pendingRecovery: 'Ожидают очистки: {count}',
          recoveryNotice: 'Очистка {count} задач не завершена. Повторите очистку окончательно неудачных задач, чтобы продолжить.',
          options: {
            active: 'Остановить выполняемые задачи',
            pending: 'Очистить ожидающие задачи',
            scheduled: 'Очистить запланированные задачи',
            retry: 'Очистить повторные попытки',
            archived: 'Очистить окончательно сбойные',
            completed: 'Очистить завершённые',
          },
          confirm: 'Обработать задачи очереди «{queue}» в состоянии «{state}». Сейчас отображается {count} задач.',
          liveWarning: 'Операция может затронуть несколько рабочих пространств и отменяет связанные операции и задачи того же документа в других очередях. Для Wiki также обрабатываются связанные задачи той же базы знаний. Записанные и удалённые данные не восстанавливаются. Задачи удаления баз знаний и индексов сохраняются для завершения очистки ресурсов. Остальные выполняемые обработчики должны завершиться; задачи, которые не удалось остановить или очистить, будут указаны. Новые задачи могут продолжать поступать.',
          historyWarning: 'Для обычных исторических задач удаляются только записи очереди. Если остались изолированные задачи после незавершённой очистки, сначала повторяется необходимое завершение. Исходные задачи не запускаются повторно, содержимое документов не восстанавливается.',
          confirmButton: 'Подтвердить',
          success: 'Очищено задач: {count}',
          partial: 'Очищено: {count}, не завершено: {failed}: {reasons}',
          error: 'Не удалось очистить задачи очереди',
          failures: {
            snapshot_missing: 'У {count} задач отсутствует исходный снимок очистки; задачи сохранены для обработки оператором',
            cleanup_required: 'Сохранено {count} задач удаления баз знаний или индексов для завершения очистки ресурсов',
            queue_quarantine_failed: 'Не удалось перевести {count} задач в безопасное изолированное состояние',
            worker_not_stopped: 'Не подтверждена остановка {count} задач',
            business_cancel_failed: 'Не удалось отменить операции для {count} задач',
            queue_delete_failed: 'Не удалось удалить записи {count} задач',
          },
        },
        failedNotice: {
      ...
      sections: {
        other: {
          ...
          description: 'Брендинг платформы и параметры развёртывания вне стандартных групп.'
        },
        security: {
  ...
  knowledgeEditor: {
    ...
    apiAccess: {
      title: 'Доступ к API',
      description: 'Синхронизируйте документы внешней системы с этой базой знаний: создание, обновление, просмотр статуса и удаление.',
      createTitle: 'Создать ключ доступа',
      createDescription: 'Назовите ключ по имени исходной системы, чтобы его было легко найти и отозвать.',
      namePlaceholder: 'Например: Система политик',
      create: 'Создать ключ',
      createdTitle: 'Ключ доступа создан',
      createdDescription: 'Скопируйте и сохраните его сейчас. После ухода со страницы полный ключ больше не отображается.',
      quickStartTitle: 'Быстрый старт',
      copyExample: 'Копировать пример',
      exampleCopySuccess: 'Пример запроса скопирован',
      sourceIdHint: 'Постоянный идентификатор исходной системы, до 128 символов, например policy-system.',
      externalIdHint: 'Уникальный идентификатор документа в исходной системе, до 512 символов. Используйте его во всех трёх операциях.',
      listTitle: 'Ключи доступа для этой базы знаний',
      loading: 'Загрузка ключей…',
      empty: 'Ключей доступа нет',
      neverUsed: 'Не использовался',
      revoke: 'Отозвать',
      revokeConfirmTitle: 'Отозвать ключ доступа',
      revokeConfirmBody: 'После отзыва «{name}» внешняя синхронизация с этим ключом сразу прекратится.',
      revokeSuccess: 'Ключ отозван',
      revokeFailed: 'Не удалось отозвать ключ',
      nameRequired: 'Введите название ключа',
      loadFailed: 'Не удалось загрузить ключи доступа',
      createFailed: 'Не удалось создать ключ доступа',
      createSuccess: 'Ключ доступа создан',
      copySuccess: 'Ключ скопирован',
      docsTab: 'Документация API',
      keysTab: 'Ключи доступа',
      authStep: 'Добавьте заголовок авторизации',
      callStep: 'Выберите и вызовите операцию',
      chooseOperation: 'Выберите операцию API',
      upsertTitle: 'Создание / обновление',
      upsertDescription: 'Загрузите файл. При совпадении идентификаторов системы и документа обновляется существующий документ; при первой загрузке создаётся новый.',
      upsertResponseHint: '202 означает, что запрос принят, а обработка продолжается в фоне. Проверяйте статус через GET. action равен created или updated; неизменившийся документ в обработке или после её завершения возвращает 200 и skipped.',
      getTitle: 'Проверка статуса',
      getDescription: 'Получите статус обработки, текущий ID документа и результат по идентификаторам системы и документа.',
      getResponseHint: 'Значение completed в parse_status означает завершение обработки. При failed проверьте error_message. Если документа нет, возвращается 404. Ответ GET не содержит поля action.',
      deleteTitle: 'Удаление документа',
      deleteDescription: 'Когда исходный документ удалён или снят с публикации, удалите его из базы знаний по тем же идентификаторам системы и документа.',
      deleteResponseHint: 'При удалении возвращаются 200 и deleted; если документа уже нет — 200 и skipped. Удаляются документ и связанные индексы. parse_status отражает состояние до удаления.',
      parametersTitle: 'Параметры запроса',
      queryParameters: 'Параметры URL',
      parameterName: 'Параметр / тип',
      parameterRequired: 'Обязательный',
      parameterDescription: 'Описание',
      required: 'Да',
      optional: 'Нет',
      fileHint: 'Файл документа. Допустимые форматы и размер определяются настройками сервера. Для обновления также нужен полный файл.',
      metadataHint: 'Объект JSON со строковыми значениями, например отдел или категория. Изменение метаданных также вызывает обновление.',
      requestExampleTitle: 'Пример запроса',
      responseExampleTitle: 'Пример ответа',
      exampleHint: 'Адрес уже содержит текущий сайт и базу знаний. Замените API_KEY и оба идентификатора; при загрузке также замените путь к локальному файлу.',
      statusesTitle: 'Статусы обработки',
      statusPending: 'Ожидает обработки',
      statusProcessing: 'Разбор, разбиение на фрагменты или векторизация',
      statusFinalizing: 'Создание резюме, вопросов или графа знаний',
      statusCompleted: 'Обработка завершена',
      statusFailed: 'Ошибка обработки. Проверьте error_message; документ можно повторно отправить через PUT.',
      statusCancelled: 'Обработка отменена. Документ можно повторно отправить через PUT.',
      errorsTitle: 'Типичные ошибки',
      error400: 'Недопустимые параметры, формат, размер файла или метаданные',
      error401: 'Ключ отсутствует или недействителен',
      error403: 'Нет разрешения ingest для этой базы знаний или доступ запрещён областью действия ключа',
      error404: 'База знаний не найдена или GET не нашёл документ',
      error409: 'Документ конфликтует с существующей записью',
      error429: 'Исчерпана квота хранилища пространства',
      error500: 'Ошибка обработки на сервере. Проверьте журналы сервиса.',
      syncRulesTitle: 'Повторные вызовы и синхронизация',
      identityRule: 'Внутри базы знаний сочетание source_id + external_id однозначно определяет документ для создания, обновления, просмотра и удаления.',
      skipRule: 'Если имя файла, содержимое и метаданные не изменились, а обработка идёт или завершена, повторный PUT вернёт skipped. После ошибки или отмены повторная отправка запускает обработку заново.',
      updateRule: 'Обновление создаёт новый документ и удаляет старый, поэтому knowledge_id меняется. Для синхронизации используйте source_id и external_id.',
      orderRule: 'Запросы для одного документа выполняются последовательно, но сервер не определяет порядок бизнес-версий. Отправляйте запросы в порядке изменений исходного документа.',
      columns: {
        name: 'Название',
        key: 'Ключ',
        createdAt: 'Создан',
  ...
  embedPublish: {
    ...
    webhookUrlPlaceholder: 'https://your-server.example.com/embed-events',
    webhookUrlDesc: 'Optional. The system POSTs message_sent / message_received events to this HTTPS endpoint.',
  ...
  createChat: {
    title: 'Ваш помощник по базе знаний — знания всегда под рукой',
  ...
  thirdPartyIntegration: {
    system: {
      navLabel: 'Сторонние приложения', title: 'Сторонние приложения',
      description: 'Регистрация доверенных систем и управление адресами возврата и доступными разрешениями.',
      create: 'Зарегистрировать', edit: 'Изменить',
      formDescription: 'Client Secret показывается один раз после создания или замены.',
      noDescription: 'Без описания', redirectCount: 'Адресов возврата: {count}',
      testConnection: 'Проверить соединение', testSuccess: 'Доступны все адреса возврата: {count}. HTTP: {statuses}',
      testFailed: 'Проверка не пройдена. Доступно {reachable}/{total}. {url}: {error}',
      testRequestFailed: 'Не удалось выполнить проверку соединения', testUnknownError: 'Неизвестная ошибка',
      deleteTitle: 'Удалить стороннее приложение',
      deleteConfirm: 'Удалить «{name}»? Связанные политики рабочих пространств, разрешения пользователей, подключения и учётные данные будут удалены без возможности восстановления.',
      deleted: 'Стороннее приложение удалено', deleteFailed: 'Не удалось удалить стороннее приложение',
      rotateSecret: 'Заменить Secret',
      rotateTitle: 'Замена Client Secret',
      rotateConfirm: 'Заменить Client Secret для «{name}»? Старый Secret сразу станет недействительным.',
      rotateFailed: 'Не удалось заменить Secret', emptyTitle: 'Нет зарегистрированных сторонних приложений',
      emptyDescription: 'Зарегистрируйте приложение, чтобы оно могло запрашивать разрешение пользователей.',
      required: 'Укажите имя приложения и хотя бы один адрес возврата', loadFailed: 'Не удалось загрузить приложения',
      saved: 'Приложение сохранено', saveFailed: 'Не удалось сохранить приложение',
    },
    fields: {
    ...
    scopes: {
      read: 'Чтение и поиск по базам знаний',
      chat: 'Вопросы и ответы по базам знаний',
    },
    secret: {
      title: 'Сохраните данные приложения', copy: 'Копировать Client Secret',
      warning: 'Client Secret показывается один раз. Сохраните его в защищённой конфигурации стороннего сервера.',
    },
    tenant: {
      navLabel: 'Сторонние подключения', title: 'Сторонние подключения',
      description: 'Управление разрешёнными приложениями и подключениями к нескольким пространствам.',
      applications: 'Политики приложений пространства',
      applicationsDescription: 'Администраторы управляют доступом приложения к текущему пространству и его разрешениями.',
      configure: 'Настроить',
      unrestrictedSummary: '{scopes} · Базы знаний определяются пространствами, выбранными пользователем',
      empty: 'Системный администратор ещё не зарегистрировал приложения', loadFailed: 'Не удалось загрузить подключения',
      saved: 'Политика пространства сохранена', saveFailed: 'Не удалось сохранить политику пространства',
    },
    connections: {
      title: 'Мои разрешённые подключения',
      description: 'Учётные данные хранятся сторонним сервером и аннулируются сразу после отзыва.',
      connected: 'Подключено', unavailable: 'Отключено',
      summary: 'Пространств: {spaces} · Баз знаний: {count} · {scopes}',
      unavailableSummary: 'Приложение отключено или ни одно выбранное пространство сейчас недоступно.',
      open: 'Открыть', revoke: 'Отозвать',
      revokeConfirm: 'Отозвать подключение к «{name}»? Его Agent сразу потеряет доступ.',
      empty: 'Вы ещё не разрешили доступ сторонним приложениям', revoked: 'Подключение отозвано',
      revokeFailed: 'Не удалось отозвать подключение',
    },
    policy: {
      title: 'Настройка {name}', description: 'Политика пространства может только сужать системные настройки приложения.',
      enabled: 'Разрешить в этом пространстве', enabledHint: 'Отключение сразу исключит это пространство из существующих подключений.',
    },
    authorization: {
      loading: 'Проверка запроса на подключение…', errorTitle: 'Не удалось подключиться',
      eyebrow: 'Авторизация стороннего приложения',
      title: 'Разрешить «{name}» подключиться к вашим базам знаний?',
      credentialNotice: 'Приложение получит отдельные учётные данные. Login Token и данные OIDC не передаются.',
      permissions: 'Приложение сможет', readTitle: 'Читать и искать базы знаний в выбранных пространствах',
      readDescription: 'Приложение получит доступ к доступным вам базам знаний в выбранных пространствах; изменения прав применяются сразу.',
      chatTitle: 'Отвечать на вопросы по базам знаний',
      chatDescription: 'Можно вызывать Q&A, но нельзя создавать, изменять или удалять содержимое.',
      selectWorkspaces: 'Выберите пространства', selectWorkspacesHint: 'Можно выбрать несколько пространств, а позже повторить авторизацию или отозвать подключение.',
      noWorkspaces: 'Нет пространств, которые сейчас разрешают это приложение',
      footerHint: 'Действующие разрешения пересчитываются при каждом запросе.',
      deny: 'Отклонить', allow: 'Подключить', submitFailed: 'Не удалось отправить решение',
      loadFailed: 'Не удалось загрузить запрос авторизации',
    },
    launch: {
      loading: 'Открываем ваши базы знаний…', errorTitle: 'Не удалось открыть подключение',
      successLabel: 'Авторизация завершена',
      title: 'Подключение установлено', description: '«{name}» получило подтверждённый вами доступ. Выберите базу знаний ниже.',
      workspaceCount: 'Разрешённые пространства', knowledgeBaseCount: 'Доступные базы знаний',
      workspaces: 'Разрешённые пространства',
      knowledgeBases: 'Выберите базу знаний',
      openHint: 'Открыть базу знаний',
      empty: 'В разрешённых пространствах нет доступных баз знаний. Авторизуйте подключение заново или обратитесь к администратору.',
      loadFailed: 'Не удалось загрузить подключение',
    },
  },
  common: {
    add: 'Добавить',
  ...
  auth: {
    ...
    redirectingToSSO: 'Перенаправление в центр аутентификации...',
    orContinueWith: 'Или продолжить с помощью',
    oidcLoginFailed: 'Ошибка входа через OIDC',
    quickLogin: 'Быстрый вход',
    quickLoginDescription: 'Выберите организацию для входа через единую аутентификацию',
    passwordLogin: 'Вход по логину и паролю',
    fushunSSOProvider: 'Fushun Xinxin Steel',
    fushunSSOLogin: 'SSO Fushun Xinxin Steel',
    fushunSSODescription: 'Войти через единую аутентификацию Fushun Xinxin Steel',
    jianlongSSOProvider: 'Группа Jianlong',
    jianlongSSOLogin: 'SSO группы Jianlong',
    jianlongSSODescription: 'Войти через единую аутентификацию группы Jianlong',
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с базой знаний',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте базы знаний; скрипты и файлы остаются в том же контейнере',
        local: 'Run scripts directly in the server process environment',
      },
      ...
      weknoraTemplateTitle: 'Standard sandbox template',
      weknoraDockerImage: 'Standard sandbox image',
      ...
      localRuntimeWarning: 'Local process runs skill scripts directly on the server without container or MicroVM isolation. Use only in a trusted development workspace.',
      weknoraTemplateOverview: 'The system provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      weknoraTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by skills.',
      ...
      localRuntimeSummary: 'Server process',
      ...
      templateLoadHint: 'Enter the cluster connection and refresh. The system creates its standard template when missing.',
      templateLoadFailed: 'Failed to load templates',
      standardTemplateProvisioning: 'The standard sandbox template is being created. Refresh shortly to see its status.',
      ...
      templateStepHint: 'The system automatically creates its standard template when missing. You can continue after a template is ready.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — система пересоберёт шаблон.',
      ...
      dockerTlsCertPathHelp: 'Directory on the server host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by the system and rebuilt when the session continues. Empty means 1800 seconds.',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: 'напр. knowledge/images',
      prefixPlaceholder: 'напр. knowledge',
    ...
    weknoraCloud: {
      ...
      description: 'Настройте учётные данные APPID и APPSECRET для облачного сервиса. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные облачного сервиса истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения облачного сервиса',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения облачного сервиса',
      ...
      addModelsDisplayName: {
        chat: 'Облачная модель чата',
        embedding: 'Облачная модель эмбеддингов',
        rerank: 'Облачная модель реранкинга',
        vllm: 'Облачная модель зрения'
      },
      modelsSection: {
        title: 'Облачные модели',
        descReady: 'Зарегистрируйте четыре стандартные модели облачного сервиса для чата, поиска, реранкинга и зрения.',
  ...
  agent: {
    ...
    editor: {
      ...
      titleModel: 'Модель создания заголовка',
      titleModelPlaceholder: 'Оставьте пустым, чтобы использовать основную модель чата',
      systemPrompt: 'System Prompt',
  ...
  contextualGuide: {
    ...
    tenantModels: {
      ...
      needDocumentModelsFirst: 'Перед созданием базы знаний настройте модель чата и модель Embedding.',
      contactSystemAdmin: 'Необходимые модели платформы не настроены. Обратитесь к системному администратору.',
      stepsAgent: {
        done: {
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и система автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
