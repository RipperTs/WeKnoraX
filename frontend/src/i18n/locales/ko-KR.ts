export default {
  ...
  userProfile: {
    ...
    changePassword: {
      label: '비밀번호 변경',
      description: '새 로그인 비밀번호를 설정합니다. 성공 시 모든 세션이 만료되어 다시 로그인해야 합니다.',
      newLabel: '새 비밀번호',
  ...
  integrations: {
    ...
    claw: {
      title: '지식 베이스 스킬',
      subtitle: '지식 베이스 REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
      ...
      installCtaHint: '지식 베이스 Skill 설치 · 새 탭에서 열림',
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 지식 베이스와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: '지식 베이스 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      capabilityManageModelsHint: '모델 설정, 자격 증명, 연결 테스트 및 클라우드 서비스 자격 증명을 관리합니다.',
      ...
      signedFlowDetail: 'HMAC secret은 지식 베이스와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=weknora, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: '지식 베이스에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
  ...
  datasource: {
    ...
    noResourcesDesc_confluence: '계정에 읽을 수 있는 Confluence 공간이 있거나 익명 읽기 권한이 활성화되어 있는지 확인하세요',
    ...
    connectorDesc: {
      ...
      confluence: 'Confluence 공간에서 페이지 및 문서 첨부 파일 동기화',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      titleModel: '세션 제목을 자동 생성할 모델입니다. 비워 두면 기본 대화 모델을 사용합니다.',
  ...
  model: {
    ...
    editor: {
      ...
      sortOrderPlaceholder: '기본값: 100',
      sortOrderDesc: '사용자 모델 선택 목록의 순서를 지정합니다. 숫자가 작을수록 앞에 표시되며, 같으면 생성 시간 역순으로 표시됩니다.',
      ...
      providers: {
        ...
        weknoracloud: {
          label: '클라우드 모델',
          description: 'chat, embedding, rerank, VLM 모델을 지원하는 클라우드 서비스'
        },
        requesty: {
          label: 'Requesty',
    ...
    globalSettings: {
      ...
      runtime: {
        ...
        purge: {
          ...
          liveWarning: '여러 워크스페이스에 영향을 줄 수 있으며 관련 업무와 동일 문서의 다른 큐 작업도 취소됩니다. Wiki는 동일 지식 베이스의 관련 작업도 처리합니다. 이미 기록하거나 삭제한 데이터는 되돌리지 않습니다. 지식 베이스 및 인덱스 삭제 작업은 리소스 정리를 완료하도록 유지됩니다. 그 외 실행 중인 처리기는 종료되어야 하며 중지 또는 정리에 실패한 작업은 표시됩니다. 새 작업은 계속 큐에 들어올 수 있습니다.',
          historyWarning: '일반 이력 작업은 큐 기록만 삭제합니다. 이전 정리가 완료되지 않은 격리 작업이 있으면 필요한 마무리를 먼저 다시 시도합니다. 원래 작업은 다시 실행하지 않으며 문서 내용은 되돌리지 않습니다.',
          confirmButton: '실행 확인',
  ...
  knowledgeEditor: {
    ...
    apiAccess: {
      title: 'API 액세스',
      description: 'API로 외부 시스템의 문서를 현재 지식베이스에 동기화합니다. 생성, 수정, 조회, 삭제를 지원합니다.',
      ...
      copyExample: '호출 예제 복사',
      exampleCopySuccess: '호출 예제가 복사되었습니다',
      sourceIdHint: '원본 시스템의 고정 식별자입니다. 최대 128자이며 예시는 policy-system입니다.',
      externalIdHint: '원본 시스템 내 문서의 고유 식별자이며 최대 512자입니다. 세 작업 모두 동일한 식별자를 사용하세요.',
      ...
      authStep: '인증 헤더 추가',
      callStep: '작업 선택 및 호출',
      chooseOperation: 'API 작업 선택',
      upsertTitle: '생성 / 수정',
      upsertDescription: '문서 파일을 업로드합니다. 원본 시스템 및 문서 식별자가 일치하면 기존 문서를 수정하고, 처음 업로드하면 새 문서를 생성합니다.',
      upsertResponseHint: '202는 요청이 접수되었음을 뜻하며 파싱은 백그라운드에서 진행됩니다. GET으로 상태를 확인하세요. action은 created 또는 updated이며, 처리 중이거나 완료된 동일한 내용은 200과 skipped를 반환합니다.',
      getTitle: '상태 조회',
      getDescription: '원본 시스템 및 문서 식별자로 파싱 진행 상황, 현재 문서 ID와 처리 결과를 조회합니다.',
      getResponseHint: 'parse_status가 completed이면 처리가 완료된 것입니다. failed이면 error_message를 확인하세요. 문서가 없으면 404를 반환하며 조회 응답에는 action 필드가 없습니다.',
      deleteTitle: '문서 삭제',
      deleteDescription: '원본 문서가 삭제되거나 게시 중단되면 동일한 원본 시스템 및 문서 식별자로 지식베이스의 문서를 삭제합니다.',
      deleteResponseHint: '삭제 성공 시 200과 deleted를 반환합니다. 문서가 없어도 200과 skipped를 반환합니다. 문서와 관련 인덱스를 삭제하며, parse_status는 삭제 전 파싱 상태입니다.',
      parametersTitle: '요청 매개변수',
      queryParameters: 'URL 쿼리 매개변수',
      parameterName: '매개변수 / 유형',
      parameterRequired: '필수',
      parameterDescription: '설명',
      required: '예',
      optional: '아니요',
      fileHint: '업로드할 문서 파일입니다. 형식과 크기 제한은 서버 설정을 따르며, 수정 시에도 전체 파일을 제출해야 합니다.',
      metadataHint: '부서나 분류 등의 문자열 값을 갖는 JSON 객체입니다. 메타데이터 변경 시에도 문서가 수정됩니다.',
      requestExampleTitle: '호출 예시',
      responseExampleTitle: '응답 예시',
      exampleHint: '주소에는 현재 사이트와 지식베이스가 반영되어 있습니다. API_KEY와 두 식별자를 바꾸고, 업로드 시 로컬 파일 경로도 바꾸세요.',
      statusesTitle: '처리 상태 안내',
      statusPending: '처리 대기 중',
      statusProcessing: '파싱, 청킹 또는 벡터화 중',
      statusFinalizing: '요약, 질문 또는 지식 그래프 생성 중',
      statusCompleted: '모든 처리 완료',
      statusFailed: '처리 실패. error_message를 확인하고 PUT으로 문서를 다시 제출할 수 있습니다.',
      statusCancelled: '처리가 취소되었습니다. PUT으로 문서를 다시 제출할 수 있습니다.',
      errorsTitle: '일반적인 오류',
      error400: '매개변수, 파일 형식, 크기 또는 메타데이터 오류',
      error401: '키가 없거나 유효하지 않음',
      error403: '현재 지식베이스의 ingest 권한이 없거나 액세스 범위 밖임',
      error404: '지식베이스가 없거나 GET으로 조회한 문서가 없음',
      error409: '기존 기록과 문서 충돌',
      error429: '현재 워크스페이스의 저장 용량 소진',
      error500: '서버 처리 실패. 서비스 로그를 확인하세요.',
      syncRulesTitle: '반복 호출 및 동기화 규칙',
      identityRule: '한 지식베이스에서 source_id + external_id로 문서를 고유하게 식별하며 생성, 수정, 조회, 삭제에 동일하게 사용합니다.',
      skipRule: '파일명, 내용, 메타데이터가 같고 처리 중이거나 완료된 경우 반복 PUT은 skipped를 반환합니다. 실패하거나 취소된 후 다시 제출하면 재처리됩니다.',
      updateRule: '수정 시 새 문서를 생성하고 기존 문서를 정리하므로 knowledge_id가 바뀝니다. 동기화에는 계속 source_id와 external_id를 사용하세요.',
      orderRule: '동일 문서의 요청은 순차 실행되지만 서버는 업무 버전의 순서를 판단할 수 없습니다. 원본 문서가 변경된 순서대로 요청하세요.',
      columns: {
        name: '이름',
        key: '키',
        createdAt: '생성 시간',
  ...
  createChat: {
    title: '안녕하세요, 지식 도우미입니다 — 당신의 지식을 손끝에',
  ...
  thirdPartyIntegration: {
    system: {
      navLabel: '서드파티 앱', title: '서드파티 앱',
      description: '신뢰할 수 있는 외부 시스템과 콜백 주소 및 요청 가능 권한을 관리합니다.',
      create: '앱 등록', edit: '앱 편집', formDescription: 'Client Secret은 생성 또는 교체 직후 한 번만 표시됩니다.',
      noDescription: '설명 없음', redirectCount: '콜백 주소 {count}개',
      testConnection: '연결 테스트', testSuccess: '콜백 주소 {count}개에 모두 연결할 수 있습니다. HTTP 상태: {statuses}',
      testFailed: '연결 테스트에 실패했습니다. {reachable}/{total}개 연결 가능. {url}: {error}',
      testRequestFailed: '연결 테스트 요청에 실패했습니다.', testUnknownError: '알 수 없는 오류',
      deleteTitle: '서드파티 앱 삭제',
      deleteConfirm: '“{name}”을 삭제하시겠습니까? 관련 스페이스 정책, 사용자 승인, 연결 및 액세스 자격 증명도 영구적으로 삭제됩니다.',
      deleted: '서드파티 앱이 삭제되었습니다.', deleteFailed: '서드파티 앱을 삭제하지 못했습니다.',
      rotateSecret: 'Secret 교체',
      rotateTitle: 'Client Secret 교체',
      rotateConfirm: '“{name}”의 Client Secret을 교체하시겠습니까? 기존 Secret은 즉시 무효화됩니다.',
      rotateFailed: 'Secret 교체 실패', emptyTitle: '등록된 서드파티 앱이 없습니다',
      emptyDescription: '앱을 등록하면 사용자 연결 승인을 요청할 수 있습니다.',
      required: '앱 이름과 하나 이상의 콜백 주소를 입력하세요', loadFailed: '앱을 불러오지 못했습니다',
      saved: '앱이 저장되었습니다', saveFailed: '앱 저장 실패',
    },
    fields: {
    ...
    secret: {
      title: '앱 자격 증명 저장', copy: 'Client Secret 복사',
      warning: 'Client Secret은 한 번만 표시됩니다. 지금 외부 서버의 안전한 설정에 저장하세요.',
    },
    tenant: {
      navLabel: '서드파티 연결', title: '서드파티 연결',
      description: '현재 워크스페이스에서 허용된 앱과 다중 워크스페이스 연결을 관리합니다.',
      applications: '워크스페이스 앱 정책', applicationsDescription: '관리자는 앱의 현재 워크스페이스 접근 여부와 권한을 제어할 수 있습니다.',
      configure: '설정',
      unrestrictedSummary: '{scopes} · 사용자가 승인한 워크스페이스의 지식 베이스를 동적으로 사용',
      empty: '시스템 관리자가 등록한 앱이 없습니다', loadFailed: '연결을 불러오지 못했습니다',
      saved: '워크스페이스 정책이 저장되었습니다', saveFailed: '워크스페이스 정책 저장 실패',
    },
    connections: {
      title: '내 승인된 연결', description: '자격 증명은 외부 서버가 보관하며 취소 즉시 무효화됩니다.',
      connected: '연결됨', unavailable: '비활성화됨',
      summary: '워크스페이스 {spaces}개 · 지식 베이스 {count}개 · {scopes}',
      unavailableSummary: '앱이 비활성화되었거나 승인된 워크스페이스를 현재 사용할 수 없습니다.',
      open: '열기', revoke: '취소',
      revokeConfirm: '“{name}” 연결을 취소하시겠습니까? 해당 Agent의 접근이 즉시 차단됩니다.',
      empty: '승인한 서드파티 앱이 없습니다', revoked: '연결이 취소되었습니다', revokeFailed: '연결 취소 실패',
    },
    policy: {
      title: '{name} 설정', description: '워크스페이스 정책은 시스템 앱 설정보다 권한을 넓힐 수 없습니다.',
      enabled: '이 워크스페이스에서 허용', enabledHint: '비활성화하면 기존 연결에서 이 워크스페이스가 즉시 제외됩니다.',
    },
    authorization: {
      loading: '연결 요청을 확인하는 중…', errorTitle: '연결할 수 없습니다',
      eyebrow: '서드파티 앱 승인',
      title: '“{name}”이 지식 베이스에 연결하도록 허용하시겠습니까?',
      credentialNotice: '앱에는 별도 자격 증명이 발급되며 로그인 Token과 OIDC 자격 증명은 공유되지 않습니다.',
      permissions: '앱 권한', readTitle: '선택한 워크스페이스의 지식 베이스 읽기 및 검색',
      readDescription: '선택한 워크스페이스에서 현재 접근 가능한 지식 베이스를 읽을 수 있으며 권한 변경은 즉시 적용됩니다.',
      chatTitle: '지식 베이스 기반 질의응답', chatDescription: '지식 Q&A는 호출할 수 있지만 콘텐츠 작성, 수정, 삭제는 할 수 없습니다.',
      selectWorkspaces: '워크스페이스 선택', selectWorkspacesHint: '여러 워크스페이스를 선택할 수 있으며 나중에 다시 승인하거나 연결을 취소할 수 있습니다.',
      noWorkspaces: '현재 이 앱을 허용하는 워크스페이스가 없습니다', footerHint: '유효 권한은 요청할 때마다 다시 계산됩니다.',
      deny: '거부', allow: '연결', submitFailed: '승인 결과 제출 실패', loadFailed: '승인 요청을 불러오지 못했습니다',
    },
    launch: {
      loading: '지식 베이스를 여는 중…', errorTitle: '연결을 열 수 없습니다',
      successLabel: '승인 완료', title: '연결되었습니다',
      description: '“{name}”에 승인한 접근 권한이 부여되었습니다. 아래에서 지식 베이스를 여세요.',
      workspaceCount: '승인된 워크스페이스', knowledgeBaseCount: '접근 가능한 지식 베이스',
      workspaces: '승인된 워크스페이스', knowledgeBases: '지식 베이스 선택', openHint: '지식 베이스 열기',
      empty: '승인된 워크스페이스에 현재 접근 가능한 지식 베이스가 없습니다. 다시 승인하거나 관리자에게 문의하세요.',
  ...
  auth: {
    ...
    redirectingToSSO: '인증 센터로 이동하는 중...',
    orContinueWith: '또는 다음 방법으로 계속',
    oidcLoginFailed: 'OIDC 로그인 실패',
    quickLogin: '빠른 로그인',
    quickLoginDescription: '소속 기업을 선택하여 통합 인증으로 로그인하세요',
    passwordLogin: '계정 및 비밀번호 로그인',
    fushunSSOProvider: '푸순 신신철강',
    fushunSSOLogin: '푸순 신신철강 SSO',
    fushunSSODescription: '푸순 신신철강 통합 인증으로 로그인합니다',
    jianlongSSOProvider: '젠룽 그룹',
    jianlongSSOLogin: '젠룽 그룹 SSO',
    jianlongSSODescription: '젠룽 그룹 통합 인증으로 로그인합니다',
    username: '사용자명',
    ...
    registerSubtitle: '계정을 만들고 지식 베이스를 사용하세요',
    loginAccountPlaceholder: '사용자명 또는 이메일 주소 입력',
    emailPlaceholder: '이메일 주소 입력',
    passwordPlaceholder: '비밀번호 입력 (8-32자, 문자와 숫자 포함)',
    confirmPasswordPlaceholder: '비밀번호 다시 입력',
    usernamePlaceholder: '사용자명 입력',
    loginAccountRequired: '사용자명 또는 이메일 주소를 입력해주세요',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 지식 베이스 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      ...
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 시스템이 다시 빌드합니다.',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: '예: knowledge/images',
      prefixPlaceholder: '예: knowledge',
    ...
    weknoraCloud: {
      ...
      addModelsDisplayName: {
        chat: '클라우드 대화 모델',
        embedding: '클라우드 임베딩 모델',
        rerank: '클라우드 리랭크 모델',
        vllm: '클라우드 비전 모델'
      },
      modelsSection: {
        title: '클라우드 모델',
        descReady: '클라우드 서비스의 네 가지 표준 모델을 등록하여 대화, 검색, 재정렬, 비전에 사용합니다.',
  ...
  agent: {
    ...
    editor: {
      ...
      titleModel: '제목 생성 모델',
      titleModelPlaceholder: '비워 두면 기본 대화 모델을 사용합니다',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 시스템이 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: '지식 베이스에 오신 것을 환영합니다',
